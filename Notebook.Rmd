...
```{r}
location_data %>% select(Location, TotalEmployment, Per1000Jobs)
```
...
- If so, how much $ would it be worth to Wichita to remedy this?
...
- How does the average Wichita dev salary compare to tech hubs, RPP adjusted?
  ...
  - Ideally, nationally, but be careful with applying person-weighted RPPs to developer-weighted average income figures. Ie, if devs tend to live in high RPP areas, using the national RPP doesn't make sense.
